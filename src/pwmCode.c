...
void PWMConfigure(void) {
    ...
    GPIO_PORTF_AFSEL_R |= (1 << 5);                                         //Set Alternate Function for PE5
    GPIO_PORTF_PCTL_R |= GPIO_PCTL_PE5_M0PWM5;                              //Set Port Control to PE5 PWM value
    GPIO_PORTF_DEN_R |= (1 << 5);                                           //Set Digital Enable for PE5
    ...
    /* Calculate Initial Value for Duty Cycle (Angle = 0) */
    duty_cycle = ((set_angle * (MAX_PULSE - MIN_PULSE)/DEG_OF_ROTATION) + MIN_PULSE) / 20;
    PWM0_2_CMPB_R = CYCLES_PER_MS_DIV_100 * (duty_cycle);                   //Set initial duty cycle
